...
| BAZOS | Bazoš.cz | bazos_scraper.py |
...
- **VZDY pred deployem: `git status --short src/` a zkontroluj `??` untracked soubory.** Nove `.cs` soubory ktere nejsou v gitu Docker nevidí → build selze nebo se tiché nasadi stara verze. `git add src/ && git commit && git push` PRED kazdym deployem bez vyjimky. Preskoceni = ghost debugging.
...
### ✅ Dokončeno v Session 27 (2026-05-14)
- [x] **Price history tracking** – `listing_price_history` tabulka: `(id, listing_id, price, recorded_at, source)`; migrace `scripts/migrate_price_history.sql` (3748 řádků backfillováno se `source='import'`); GIN index na `(listing_id, recorded_at DESC)`
- [x] **view_count + date_created_source** – nové sloupce na `listings`: `view_count INTEGER`, `date_created_source TIMESTAMPTZ`; SReality extrahuje je v `_merge_detail()` ze `_embedded.stats.views` resp. `date_of_creation`
- [x] **Scraper upsert loguje cenové změny** – před upsert se načte `old_price`; po upsert INSERT do `listing_price_history` pokud `new_price != old_price`; loguje směr `⬇/⬆` a procenta
- [x] **API endpoint** – `GET /api/listings/{id}/price-history` → `List<PriceHistoryDto>(Price, RecordedAt, Source)`; raw SQL query `SqlQueryRaw<PriceHistoryRow>` (ASC order)
- [x] **Blazor UI v ListingDetail.razor** – sekce "Historie cen" se `MudSimpleTable` (datum/cena/změna), trend chip (`GetPriceTrendColor/Icon/Label`), zobrazena jen pokud `_priceHistory.Count > 0`
- [x] **ListingPriceHistory entita** – `src/RealEstate.Domain/Entities/ListingPriceHistory.cs`; EF Core konfigurace v `RealEstateDbContext.cs` (ToTable, HasColumnName, HasOne FK, HasIndex)
- [x] **Docker rebuild** – api + app + scraper rebuildy úspěšné, všechny 3 kontejnery Recreated + Up

**Last Updated:** 14. května 2026 (Session 27)
**Current Commit:** (pending) – feat(price-history): track price changes per listing + SReality view_count
**DB stav:** 1558 inzerátů, 13 zdrojů, GPS: 97 % pokrytí, AI: normalize 91 %, smart-tags 91 %, price-signal 79 % (= 100 % zpracovatelných dat); price history: 3748 řádků (backfill)
